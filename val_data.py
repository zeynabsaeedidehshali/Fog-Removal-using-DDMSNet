    ...
    def __init__(self, val_data_dir, dataset_name='kitti'):
        ...
        if self.dataset_name == 'snow100k':
            for subset in self.subsets:
                val_list_gt_path.append(os.listdir('gt/'))
                val_list_syn_path.append(os.listdir('foggy/'))
        else:
            for subset in self.subsets:
                val_list_gt_path.append(os.listdir(val_data_dir  + 'gt/'))
                val_list_syn_path.append(os.listdir(val_data_dir + 'foggy/'))
        self.haze_names = val_list_syn_path
        self.gt_names = val_list_gt_path
        self.val_data_dir = val_data_dir

    def get_images(self, index):
        haze_name = [self.haze_names[i][index] for i in range(1)]
        #print(haze_name)
        gt_name = [self.gt_names[i][index] for i in range(1)]

        haze_img = []
        gt_img = []
        if self.dataset_name == 'snow100k':
            for i, subset in enumerate(self.subsets):
                haze_img.append(Image.open(self.val_data_dir + subset + 'foggy/' + haze_name[i]))
                gt_img.append(Image.open(self.val_data_dir + subset + 'gt/' + gt_name[i]))
        else:
            for i, subset in enumerate(self.subsets):
                haze_img.append(Image.open(self.val_data_dir + 'foggy/' + haze_name[i]))
